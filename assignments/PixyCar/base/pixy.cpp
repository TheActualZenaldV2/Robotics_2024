...
void loop(){ 
   ...
   // uint16_t    blocks;                       // Create an unsigned int to hold the number of found blocks
    ...
    int         myPixyInt, myPixyIntY, myPixyIntWidth;
  ...
  if (pixy.ccc.numBlocks) {
    ...
    myPixyIntY = pixy.ccc.blocks[0].m_y;    // y location of the main object    0 - 320
    myPixyIntWidth = pixy.ccc.blocks[0].m_width;    // width of the main object    0 - 320

    Serial.print("X Location: ");
    Serial.println(myPixyInt);  

    Serial.print("Y Location: ");
    Serial.println(myPixyIntY);  

    Serial.print("Width: ");
    Serial.println(myPixyIntWidth);  

  }  else {  // no blocks seen by camera
    Serial.println("Pixy No Object");       
  }
           
   delay(100); // just for the heck of it slow things down a bit
}
