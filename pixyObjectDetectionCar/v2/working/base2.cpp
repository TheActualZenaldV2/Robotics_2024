...
{  
  ...
  tiltLoop.m_command = constrain(tiltLoop.m_command, 0, 800);
    ...
    Serial.print(tiltLoop.m_command); // Print the label
...
Serial.println(mainServoValue); // Print the main servo value
 ...
 digitalWrite(D5, 1); // Set one direction
   ...
   // Pan and tilt by incrementing angles
    panLoop.m_command += 10; // Increase pan angleA
    tiltLoop.m_command += 10; // Increase tilt angle

    // Ensure pan and tilt angles stay within limits
    panLoop.m_command = panLoop.m_command % 1000; // Keep within 0-600 range
    tiltLoop.m_command = tiltLoop.m_command % 800; // Keep within 0-600 range
